...
public class RewriteListener implements Listener {
    ...
    public void onRewriteWeapon(ItemRewriteEvent event) {
        ...
        if(json.hasDynamicValue("AttackWidth")) lore.add(ChatColor.BLUE + "巻き込み: " + formatDouble(json.getDynamicValue("AttackWidth").getAsMaskedDouble().orElse(0.0) * 100) + "%");
    ...
    public void onRewriteArmor(ItemRewriteEvent event) {
        ...
        lore.add("");
        GrassJson json = event.getJson();
        if(json.hasDynamicValue("Defence")) lore.add(ChatColor.BLUE + "防御: " + json.getDynamicValue("Defence").getAsMaskedInteger().orElse(0));
        if(json.hasDynamicValue("Protection")) lore.add(ChatColor.BLUE + "保護: " + json.getDynamicValue("Protection").getAsMaskedInteger().orElse(0));
        if(json.hasDynamicValue("MagicDefence")) lore.add(ChatColor.BLUE + "魔法防御: "+ json.getDynamicValue("MagicDefence").getAsMaskedInteger().orElse(0));
        if(json.hasDynamicValue("BonusAttackDamage")) lore.add(ChatColor.BLUE + "追加攻撃力: "+ json.getDynamicValue("BonusAttackDamage").getAsMaskedDouble().orElse(0.0));
        event.setLore(lore);
        event.setShowable(true);
    }
